...
- `app/` — Django project root (mounted into container at `/app`)
  ...
  - `templates/` — HTML templates (base layout + 28 mining app templates)
  - `static/css/theme.css` — single CSS file for the entire theme
- `media/` — user-uploaded miner/platform images (bind mount)
- `staticfiles/` — collected static files for Nginx/Gunicorn (bind mount, gitignored)
...
- `DEPLOYMENT.md` — production deployment guide (Nginx, TLS, LAN/VPS)
